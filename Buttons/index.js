...
export function Button(props) {
  ...
  const modalType = {
    ...
    testDrive: modals.TestDriveModalChangeVisibility,
  };
...
export function VideoButton(props) {
  ...
  return (
    <>
      <button
        className={videoButtonTheme[props.style || "normal"].style}
        aria-label="Play video"
        style={props.uppercase ? { textTransform: "uppercase" } : null}
        onClick={() => modals.VideoModalOpen(props.link)}
      >
        {videoButtonTheme[props.style || "normal"].playButton ? playIcon : ""}
        <div className={style.text}>{props.text || "Watch video"}</div>
...
export function DefaultButton(props) {
  return (
    <button className={style.default_button} aria-label="Default button">
...
const playIconCircleFill = (
  <svg
    ...
    viewBox="0 0 24 24"
    fill="none"
    className={style.play_icon_circle_fill}
  >
    <path
