    ...
    >
      ...
      <div className={`${style.slider__cell} ${style.cell_info}`}>
        <div className={style.top}>
          <img
            key={activeCell}
            className={`${style.logo} zoom-animation`}
            src={data[activeCell].logo}
            alt="logo"
          />
        ...
        <div className={style.text_block} key={activeCell}>
          <Fade direction="up" triggerOnce>
            <h3 className={style.title}>{data[activeCell].title}</h3>
            <p className={style.text}>{data[activeCell].text}</p>
          </Fade>
        </div>
        <div className={style.buttons}>
          <button
            className={`${style.button_prev} zoom-animation`}
            onClick={Prev}
          >
            {arrow}
          </button>
          <button
            className={`${style.button_next} zoom-animation animated-second`}
            onClick={Next}
          >
            <span>Next</span>
            {arrow}
...
const arrow = (
  ...
  >
    <path
      d="M10.85 22L13.0355 19.7843L5.9365 12.5714L31 12.5714L31 9.42857L5.9365 9.42857L13.051 2.21571L10.85 -1.76157e-06L9.61651e-07 11L10.85 22Z"
      fill="white"
    />
  </svg>
);
